		...
		if ($(window).width() > 600){
			...
			$.getJSON(url, function(json){
				...
				for (var i=0; i<length; i++){
					...
					//item.className = 'balance-text'
					//$(item).balanceText();
					var j = 81;
					if (json[i].text.length > j) {
						var j = 81;
						while (!isBreakOpportunity(json[i].text,j)) {
							j++;
						}
						link_text = json[i].text.substr(0,j) + '<br />' + json[i].text.substr(j);
					} else {
						link_text = json[i].text
					}

					item.innerHTML = linkify(link_text) + '<small>'+relativeDate(json[i].created_at)+'</small>';
